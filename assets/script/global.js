var Global = cc.Class({
    ...
    statics: {
         ...
         * @return cc.v2       垂足坐标
         */
        getFootOfPerpendicular(point,p1,p2){
            //计算直线公式一般式 Ax+By+c=0
            let A = p2.y - p1.y
            let B = p1.x - p2.x
        ...
        getNumString(num,l=4){
            let s = num.toString();
            let n = parseInt((s.length-l)/3);
            let dw = "KMBTXXX";
            if(n>0) s = s.slice(0,s.length-n*3) + dw[n-1];
            return s;           
        },
        ...
        showmessage: function (title, content,callback=null,yes="确 定",no=null,nocallback=null) {
            cc.loader.loadRes("prefab/alertbox", function (err, prefab) {
                var msg = cc.instantiate(prefab);
        ...
        showTip: function(content,time=1.5){
             cc.loader.loadRes("prefab/alertbox", function (err, prefab) {
                var msg = cc.instantiate(prefab);
                cc.director.getScene().getChildByName("Canvas").addChild(msg);
        ...
        saveUserData(key,value,callback=null){
            if(!cc.gg.online) return;
            let data = {
                account:cc.gg.userData.account,
            ...
            cc.gg.http.sendRequest("/setinfo",data,function(ret){
                ...
                if(callback){
                    if(ret.errcode==0)callback(true);
                    else callback(false);
                }                
