...
cc.Class({
    ...
    properties: {
        ...
        wayid:0,
        speed:300,
        state:0, //状态:0 停车状态,1:运行状态
        coin_quan:0,
        pb_hecheng:cc.Prefab,
        bsetime:1,
        angle:null,
        js_time:0,
        js_num:0,
    ...
    copy(){
        ...
        this.car_copy.parent = this.node.parent;
    ...
    setSkin(){
        ...
        this.coin_quan = parseInt(cc.gg.coin[this.carid]/cc.gg.level_times[this.carid]);
        this.speed = 3950/cc.gg.level_times[this.carid];       

    },

    onTouchStart(event){        
        if(this.b_isRunning || cc.gg.touchid>-1)return;
        cc.gg.sceneNode.emit("car_touch_start",this);
        this.copy();
        cc.gg.touchid = this.index;
    },
    onTouchMove(event){
        if(this.b_isRunning || cc.gg.touchid !=this.index)return;
    ...
    onTouchEnd(event){
        ...
        this.node.zIndex = 1;
        cc.gg.sceneNode.emit("car_touch_end",{car:this,pos:this.node.getPosition()});
    },
    //复位
    replace(){
    ...
    upgrade(){
        ...
        this.node.runAction(cc.sequence(cc.delayTime(0.2), cc.spawn(cc.scaleTo(0.2,0.8),cc.fadeTo(0.2,0)),cc.callFunc(()=>{
            cc.gg.audioMgr.playSFX("hecheng");
            self.carid++;
            self.setSkin();
            self.node.scale = 0.1;
            self.node.opacity=255;
            //显示粒子效果
            let node = cc.instantiate(this.pb_hecheng);
    ...
    run(){
        ...
        this.car_copy.on("touchstart",event=>{
            ...
            cc.gg.sceneNode.emit("car_stop",this.coin_quan);
            this.node.scaleX = 1;
            this.node.scaleY = 1;
            this.replace();            
            this.wayid = 0;
    ...
    update(dt){
        ...
        //计算下一点坐标
        let d = dt*this.speed* (cc.gg.jiasu + this.js_num);
        let p = this.node.getPosition();
        switch(this.wayid){
            case 0://左边,向上
                p.y+=d;
                if(p.y<cc.gg.way[this.wayid].y){
                  this.node.setPosition(p);
                  break;
                }
                //超出线段范围,则判断角度  cc.gg.way[1] : 左线段上面y坐标和圆半径
                let radian = Math.atan((p.y-cc.gg.way[1].y)/cc.gg.way[1].r);
                let angle = radian*180/3.14;
                this.angle = 270 - angle;

                var hudu = (2*Math.PI / 360) * this.angle;
                var x = 0 + Math.sin(hudu) * cc.gg.way[1].r;
                var y = cc.gg.way[1].y - Math.cos(hudu) * cc.gg.way[1].r;
            ...
            case 1://上面半圆
                ...
                if(this.angle<90){
                    ...
                    this.node.setPosition(cc.v2(cc.gg.way[2].x,p.y-d))
                    this.wayid = 2;
                    break;
                }
                var hudu = (2*Math.PI / 360) * this.angle;
            ...
            case 2://右边向下
                ...
                //超出线段范围,则判断角度  cc.gg.way[3] : 左线段上面y坐标和圆半径
                //console.log(p);
                radian = Math.atan((p.y-cc.gg.way[3].y)/cc.gg.way[3].r);
                angle = radian*180/3.14;
                this.angle = 90 - angle;
                //console.log(angle,this.angle);

                hudu = (2*Math.PI / 360) * this.angle;
                x = 0 + Math.sin(hudu) * cc.gg.way[3].r;
                y = cc.gg.way[3].y + Math.cos(hudu) * cc.gg.way[3].r;
                this.node.rotation = 180-angle;
                this.node.setPosition(cc.v2(x,y)) ;                
                this.wayid = 3;
                //增加金币
                cc.gg.sceneNode.emit("gold_add",this.carid);
                break;
            case 3://下面半圆
                this.angle += d*0.20328; //1像素=0.20328度
                if(this.angle>270){
                    this.node.rotation = 0;
                    this.node.setPosition(cc.v2(cc.gg.way[0].x,p.y+d))
                    this.wayid = 0;
                    break;
                }
                hudu = (2*Math.PI / 360) * this.angle;
...
/*
        ...
        let x,y;
        let isleft = [0,1,2,3,4,11].indexOf(this.wayid)>=0;
